...
def classify_email(email_content):
    ...
    # Calculate ham score
    for keyword, weight in INITIAL_HAM_KEYWORDS.items():
        if keyword in email_content:
            ham_score += weight
...
class SpamDetectorApp:
    def __init__(self, root):
        ...
        # Title Label
        self.title_label = tk.Label(root, text="Spam Detector", font=("Arial", 16, "bold"))
        self.title_label.pack(pady=10)

        # Text Area for Email Input
        self.email_input = scrolledtext.ScrolledText(root, width=60, height=15, wrap=tk.WORD)
